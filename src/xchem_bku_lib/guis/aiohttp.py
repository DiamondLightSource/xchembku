...
# Object managing xchembku_composers.
from xchembku_lib.composers.composers import (
    Composers,
    xchembku_composers_get_default,
    xchembku_composers_has_default,
    xchembku_composers_set_default,
)

# Gui protocolj things (must agree with javascript).
from xchembku_lib.guis.constants import Commands, Cookies, Keywords

logger = logging.getLogger(__name__)

thing_type = "xchembku_lib.xchembku_guis.aiohttp"


# ------------------------------------------------------------------------------------------
class Aiohttp(Thing, BaseAiohttp):
    """
    Object implementing remote procedure calls for xchembku_gui methods.
    """
    ...
    async def activate_coro(self):
        ...
        try:
            ...
            if not xchembku_composers_has_default():
                # The xchembku_composer to use.
                xchembku_composer_specification = {
                    "type": "xchembku_lib.xchembku_composers.html"
                }

                # Set up the default xchembku_composer.
                xchembku_composer = Composers().build_object(
                    xchembku_composer_specification
                )
                xchembku_composers_set_default(xchembku_composer)
    ...
    async def _fetch_image_list(self, opaque, request_dict):
        ...
        html = xchembku_composers_get_default().compose_image_list(records)
        filters = {
