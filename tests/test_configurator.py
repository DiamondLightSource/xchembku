...
# Configurator.
from xchembku_lib.configurators.configurators import Configurators
...
class TestGoodConfigurationEnv:
    def test(self, constants, logging_setup, output_directory):
        ...
        # ---------------------------------------------------
        environ = {
            Envvar.XCHEMBKU_CONFIGFILE: good_config_filename,
        }
...
class TestBadConfigurationEnv:
    def test(self, constants, logging_setup, output_directory):
        ...
        BadConfigurationEnvTester(
            "RuntimeError", "XCHEMBKU_CONFIGFILE is not set"
        ).main(
            constants,
            {},
            output_directory,
        ...
        BadConfigurationEnvTester("RuntimeError", "does_not_exist.yaml").main(
            constants,
            {
                Envvar.XCHEMBKU_CONFIGFILE: "does_not_exist.yaml",
            },
            output_directory,
        )
...
class GoodConfiguratorDirectTester(Base):
    ...
    async def _main_coroutine(self, constants, specification, output_directory):
        ...
        xchembku_configurator = Configurators().build_object(specification)

        # Perform symbol replacement.
        xchembku_configurator.substitute({"output_directory": output_directory})

        # Remove top level key of this name.
        xchembku_configurator.remove(["key_to_remove"])

        # Get the current dict representation of the configuration.
        current = await xchembku_configurator.load()
        ...
        value = xchembku_configurator.require("multi.level1.level2")
        assert value == "value_at_level2"

        # Check a good error on non-existence of key.
        with pytest.raises(NotFound):
            value = xchembku_configurator.require("multi.level1.level3")

        # Check a good error on non-existence of key.
        with pytest.raises(NotFound):
            value = xchembku_configurator.require("multi.level1.level2.level3")

        # Substitute in token.
        value = xchembku_configurator.resolve(
...
class BadConfiguratorDirectTester(Base):
    ...
    async def _main_coroutine(self, constants, specification, output_directory):
        ...
        try:
            ...
            await xchembku_configurator.load()

            logger.info(describe("loaded yaml", xchembku_configurator.get_current()))
        except Exception as exception:
